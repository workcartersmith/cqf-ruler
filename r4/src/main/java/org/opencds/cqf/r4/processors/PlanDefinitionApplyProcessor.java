...
public class PlanDefinitionApplyProcessor {
    ...
    private void resolveDefinition(Session session, PlanDefinition.PlanDefinitionActionComponent action) {
        if (action.hasDefinition()) {
            PlanDefinitionApplyProvider.logger.debug("Resolving definition "+ action.getDefinitionCanonicalType().getValue());
            ...
            if (definition.startsWith(session.getPlanDefinition().fhirType())) {
                ...
                try {
                    ...
                    PlanDefinitionApplyProvider.logger.error("ERROR: ActivityDefinition %s could not be applied and threw exception %s", action.getDefinition(), e.toString());
                }
            }
        }
    }
    ...
    private Boolean meetsConditions(Session session, PlanDefinition.PlanDefinitionActionComponent action) {
        ...
        for (PlanDefinition.PlanDefinitionActionConditionComponent condition: action.getCondition()) {
            ...
            if (condition.hasExpression() && condition.getExpression().hasDescription()) {
                PlanDefinitionApplyProvider.logger.info("Resolving condition with description: " + condition.getExpression().getDescription());
            }
            if (condition.getKind() == PlanDefinition.ActionConditionKind.APPLICABILITY) {
                if (condition.hasExpression() && condition.getExpression().hasLanguage() && !condition.getExpression().getLanguage().equals("text/cql") && !condition.getExpression().getLanguage().equals("text/cql.name")) {
                    PlanDefinitionApplyProvider.logger.warn("An action language other than CQL was found: " + condition.getExpression().getLanguage());
                    continue;
                }
                ...
                PlanDefinitionApplyProvider.logger.info("Evaluating action condition expression " + condition.getExpression());
                ...
                if (!(result instanceof Boolean)) {
                    PlanDefinitionApplyProvider.logger.warn("The condition returned a non-boolean value: " + result.getClass().getSimpleName());
                    continue;
                }

                if (!(Boolean) result) {
                    PlanDefinitionApplyProvider.logger.info("The result of condition expression %s is false", condition.getExpression());
                    return false;
                }
            }
        }
