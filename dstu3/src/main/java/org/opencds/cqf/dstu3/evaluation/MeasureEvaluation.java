...
public class MeasureEvaluation {
    ...
    private Iterable<Resource> evaluateCriteria(HashSet<Object> evaluatedResourceSet, Patient patient, Measure.MeasureGroupPopulationComponent pop) {
        ...
        if (evaluatedResourceSet != null) {
            response.evaluationResult.expressionResults.entrySet().stream()
                .filter(expressionResult -> expressionResult.getValue() instanceof Resource)
                .forEach(expressionResult -> evaluatedResourceSet.add((Resource)expressionResult.getValue()));
        }
        Object result = response.evaluationResult.forExpression(pop.getCriteria());
        if (result == null) {
            return Collections.emptyList();
        }
    ...
    private MeasureReport evaluate(Measure measure, List<Patient> patients, MeasureReport.MeasureReportType type)
    {
                ...
                case CONTINUOUSVARIABLE: {
                    ...
                    for (Patient patient : patients) {
                        ...
                        if (inInitialPopulation) {
                            ...
                            boolean inMeasurePopulation = evaluatePopulationCriteria(null, patient,
                            ...
                            if (inMeasurePopulation) {
                                ...
                                for (Resource resource : evaluateCriteria(null, patient, measureObservationCriteria)) {
